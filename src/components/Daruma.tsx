import { useEffect } from 'react';
import {
  animate,
  motion,
  useMotionValue,
  useReducedMotion,
  useScroll,
  useSpring,
  useTransform,
  useVelocity,
} from 'motion/react';

interface DarumaProps {
  size?: number;
}

/**
 * Faux-3D daruma that rocks with the scroll and nudges itself now and then.
 *
 * A real daruma is bottom-weighted: push it and it always rights itself,
 * overshooting a couple of times on the way back ("nanakorobi yaoki" —
 * seven times down, eight times up). That physics is the whole animation:
 *
 *  - scroll VELOCITY (not position) tips it off its base, so it reacts to
 *    how hard you scroll and settles once you stop;
 *  - a deliberately under-damped spring supplies the wobble on the return;
 *  - depth comes from layering rather than real 3D geometry — the face and
 *    the specular highlight shift against the body as it turns, and a
 *    contact shadow squashes as the doll lifts off its edge.
 */
export function Daruma({ size = 30 }: DarumaProps) {
  const reduceMotion = useReducedMotion();
  const { scrollY } = useScroll();
  const scrollVelocity = useVelocity(scrollY);

  // Occasional self-nudge, kept separate so it can stack with the scroll tilt
  const idleKick = useMotionValue(0);

  const scrollTilt = useTransform(scrollVelocity, [-2600, 0, 2600], [13, 0, -13], {
    clamp: true,
  });

  const targetTilt = useTransform(
    [scrollTilt, idleKick],
    ([scroll, kick]: number[]) => scroll + kick,
  );

  // Low damping is the point: this is what produces the righting wobble.
  const tilt = useSpring(targetTilt, { stiffness: 90, damping: 7, mass: 0.9 });

  // --- Faux-3D derivations, all driven off the single tilt value ---------
  // Turning slightly around Y as it rocks stops it reading as a flat sticker.
  const rotateY = useTransform(tilt, (t) => t * 0.85);
  // The face sits "in front" of the body, so it lags the rotation.
  const faceParallax = useTransform(tilt, (t) => t * -0.16);
  // The highlight is the frontmost layer, so it travels furthest.
  const glintParallax = useTransform(tilt, (t) => t * -0.34);
  // Base slides out from under the doll and the shadow narrows.
  const shadowScaleX = useTransform(tilt, (t) => 1 - Math.min(Math.abs(t), 16) / 34);
  const shadowOpacity = useTransform(tilt, (t) => 0.34 - Math.min(Math.abs(t), 16) / 70);
  const shadowX = useTransform(tilt, (t) => t * 0.12);

  useEffect(() => {
    if (reduceMotion) return;

    let timeoutId: number;
    let controls: { stop: () => void } | undefined;

    const schedule = () => {
      // Irregular cadence so it never feels like a metronome
      timeoutId = window.setTimeout(
        () => {
          const direction = Math.random() > 0.5 ? 1 : -1;
          const strength = 6 + Math.random() * 5;
          controls = animate(idleKick, [0, direction * strength, 0], {
            duration: 1.6,
            ease: 'easeInOut',
          });
          schedule();
        },
        4200 + Math.random() * 5200,
      );
    };

    schedule();

    return () => {
      window.clearTimeout(timeoutId);
      controls?.stop();
    };
  }, [idleKick, reduceMotion]);

  return (
    <div
      className="daruma-stage relative shrink-0 select-none"
      style={{ width: size, height: size }}
      title="Daruma — siete veces caes, ocho veces te levantas"
    >
      {/* Contact shadow lives outside the figure so it stays on the ground */}
      <motion.div
        className="daruma-contact-shadow absolute left-1/2 bottom-[1px] -translate-x-1/2 rounded-[50%] bg-[#141518] blur-[1.5px]"
        style={{
          width: size * 0.52,
          height: size * 0.1,
          scaleX: shadowScaleX,
          opacity: shadowOpacity,
          x: shadowX,
        }}
        aria-hidden="true"
      />

      <motion.svg
        viewBox="0 0 64 64"
        width={size}
        height={size}
        className="daruma-figure relative block overflow-visible"
        style={{ rotate: tilt, rotateY }}
        aria-hidden="true"
      >
        <defs>
          {/* Off-centre light source is what gives the body its roundness */}
          <radialGradient id="darumaBody" cx="36%" cy="26%" r="82%">
            <stop offset="0%" stopColor="#e86353" />
            <stop offset="42%" stopColor="#cf2f22" />
            <stop offset="78%" stopColor="#a71a11" />
            <stop offset="100%" stopColor="#750f09" />
          </radialGradient>

          <radialGradient id="darumaFace" cx="42%" cy="32%" r="78%">
            <stop offset="0%" stopColor="#fffaf0" />
            <stop offset="65%" stopColor="#f3e7d3" />
            <stop offset="100%" stopColor="#dcc9ad" />
          </radialGradient>

          {/* Rim light down the shadowed edge — sells the curvature */}
          <linearGradient id="darumaRim" x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor="#ff9d8f" stopOpacity="0" />
            <stop offset="45%" stopColor="#ff9d8f" stopOpacity="0.85" />
            <stop offset="100%" stopColor="#ff9d8f" stopOpacity="0" />
          </linearGradient>
        </defs>

        {/* ---- LAYER 1: body ------------------------------------------- */}
        <path
          d="M32 7C41 7 47.5 13.5 49 22c2.5 12 3 31-17 31S11.5 34 15 22C16.5 13.5 23 7 32 7Z"
          fill="url(#darumaBody)"
        />

        {/* Occlusion at the base, so it looks like it's resting on weight */}
        <ellipse cx="32" cy="50" rx="17" ry="4.5" fill="#5e0b06" opacity="0.35" />

        {/* ---- LAYER 2: face plate + features (parallax) --------------- */}
        <motion.g style={{ x: faceParallax }}>
          <ellipse cx="32" cy="27" rx="13.6" ry="12.2" fill="url(#darumaFace)" />

          {/*
            One eye painted, one blank: the wish has been made and is still
            waiting. Convention fills the daruma's OWN left eye first — the
            one on the viewer's right — and the other once it comes true.
          */}
          <ellipse
            cx="26.2"
            cy="26.4"
            rx="3.5"
            ry="4.3"
            fill="#fdfaf4"
            stroke="#2a1a12"
            strokeWidth="1.15"
          />

          <ellipse
            cx="37.8"
            cy="26.4"
            rx="3.5"
            ry="4.3"
            fill="#1c110c"
            stroke="#2a1a12"
            strokeWidth="1.15"
          />
          {/* Catchlight, placed to agree with the body's light source */}
          <ellipse cx="36.5" cy="24.6" rx="1.15" ry="0.85" fill="#fdfaf4" opacity="0.7" />

          {/* Heavy brows */}
          <path
            d="M21.4 19.6c2.4-2.4 6.3-2.2 8.2.3"
            stroke="#241611"
            strokeWidth="2.3"
            strokeLinecap="round"
            fill="none"
          />
          <path
            d="M34.4 19.9c1.9-2.5 5.8-2.7 8.2-0.3"
            stroke="#241611"
            strokeWidth="2.3"
            strokeLinecap="round"
            fill="none"
          />

          {/* Moustache */}
          <path
            d="M26.4 33.6c2.6 2.2 8.6 2.2 11.2 0"
            stroke="#241611"
            strokeWidth="1.9"
            strokeLinecap="round"
            fill="none"
          />
        </motion.g>

        {/* ---- LAYER 3: gold belly panel ------------------------------- */}
        <motion.g style={{ x: faceParallax }} opacity="0.92">
          <ellipse
            cx="32"
            cy="44.5"
            rx="7.8"
            ry="5.2"
            fill="none"
            stroke="#e9c463"
            strokeWidth="1.35"
          />
          <path
            d="M28.6 43.4h6.8M30 46.4h4"
            stroke="#e9c463"
            strokeWidth="1.25"
            strokeLinecap="round"
          />
        </motion.g>

        {/* ---- LAYER 4: rim light + specular highlight (frontmost) ----- */}
        <path
          d="M49 22c2.5 12 3 31-17 31"
          stroke="url(#darumaRim)"
          strokeWidth="1.7"
          fill="none"
        />

        <motion.ellipse
          cx="21.5"
          cy="15.5"
          rx="5.4"
          ry="3.4"
          fill="#ffffff"
          opacity="0.3"
          transform="rotate(-30 21.5 15.5)"
          style={{ x: glintParallax }}
        />
      </motion.svg>
    </div>
  );
}
