  ...
  useEffect(() => {
    ...
    */
    ...
    const draw = (now: number) => {
      ...
      const amp = 30 + 18 * Math.sin((elapsed * Math.PI * 2) / 34);
      const wavelength = 170 + 70 * Math.sin((elapsed * Math.PI * 2) / 53 + 1.3);
      ...
      const heightAt = (x: number) => {
        const drift = elapsed * 7;
        const localAmp = amp * (0.5 + 0.5 * wobble(x * 0.6 + drift));
        const skew = 1.15 * Math.sin(x * 0.0042 + elapsed * 0.05);
        const baseline =
          11 * Math.sin(x * 0.0034 + elapsed * 0.07) +
          5 * Math.sin(x * 0.0021 - elapsed * 0.05);
        return mid + baseline + localAmp * Math.sin(x * k + phase + skew);
      };
      ...
      const buildHalf = (from: number, to: number, cutAt: 'end' | 'start') => {
        ...
        const spine = xs.map((x) => {
          ...
          const kick =
            dist < TAIL ? dir * p * 22 * (1 - Math.max(dist, 0) / TAIL) ** 2 : 0;
